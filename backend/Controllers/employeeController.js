...
export const updateEmployee = async (req, res) => {
  try {
    const { panNumber, status, ...otherFields } = req.body;
    ...
    // If PAN number is being updated, validate it
    if (panNumber) {
      const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
      if (!panRegex.test(panNumber.toUpperCase())) {
        return res.status(400).json({ message: 'Please enter a valid PAN number (e.g., ABCDE1234F)' });
      ...
      // Check if PAN number already exists for another employee
      const existingEmployeeWithPan = await Employee.findOne({
        panNumber: panNumber.toUpperCase(),
        employeeId: { $ne: req.params.id } // Exclude current employee
      });
